...
if ($verproy == 0) {
	...
	if (count ( $files ) == 0) {
		...
		foreach ( $files as $file ) {
			...
			//como moodle guarda dos elementos por archivo subido, uno con tamaño de archivo 0, que por ende no podemos leer y no nos sirve, filtro
	...
	$userid = $proyecto->userid; // obtengo el id del usuario dueÃ±o del proyecto en forma de string
	...
	$responsable = $DB->get_record_sql ( 'SELECT firstname,lastname,description FROM {user} WHERE id = ?', $userid ); // Busco el nombre del dueÃ±o del proyecto
	...
	// muestro los datos del dueño del proyecto en una tabla especial.
	echo '<table class="tg">
  <tr>
